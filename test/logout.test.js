...
it('should not logout with invalid access token', async () => {
  ...
  const res = await chai.request(app).delete('/api/user/logout').set({ "Authorization": `Bearer ${token}12` })
...
it('should logout if already logged in', async () => {
  ...
  const res = await chai.request(app).delete('/api/user/logout').set({ "Authorization": `Bearer ${token}` })
...
it('should not logout when logged out', async () => {
  const res = await chai.request(app).delete('/api/user/logout').set({ "Authorization": `Bearer ${token}` })
  expect(res.body).to.have.property('status', 401);
  expect(res.body).to.have.property('Error', 'You are logged out! Please Log in');
