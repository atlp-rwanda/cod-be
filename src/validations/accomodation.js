...
const newFacility=Joi.object().keys({
    ...
    description:Joi.string().required().min(5).label('Description is required'),
    accomodation:Joi.number().integer().min(1).label('Accomodation is required and must be valid'),
})
...
const newFacilityComponent=Joi.object().keys({
    facility:Joi.number().required().integer().min(1).label('Facility is required and must be valid'),
    description:Joi.string().required().min(5).label('Description is required'),
    image:Joi.string().required().label('Image path is required'),
    quantity:Joi.number().required().integer().min(1).label('Quantity is should be a valid number not zero'),
    price:Joi.string().required().label('Price is required'),
    allowBooking:Joi.string().required().valid('true', 'false').required().label('Please add booking status, either true or false'),
})

const updateComponent=Joi.object().keys({
    facility:Joi.number().integer().min(1).label('Facility is required and must be valid'),
    description:Joi.string().min(5).label('Description is required'),
    image:Joi.string().required().label('Image path is required'),
    quantity:Joi.number().integer().min(1).label('Quantity is should be a valid number not zero'),
    price:Joi.string().required().label('Price is required'),
    allowBooking:Joi.string().valid('true', 'false').label('Please add booking status, either true or false'),
})
...
export {newAccomodation,newFacility,newFacilityComponent,
    updateComponent,updateAccomodation, updateFacility}
