...
const addAccomodation=(req,res,next)=>{
    ...
    if (!validateAccomodation.error) {
        ...
        ApplicationError.validationError({data:{message:validateAccomodation.error.details[0].context.label}},res);
    }
};
const addfacility=(req,res,next)=>{
    ...
    if (!validateFacility.error) {
        ...
        ApplicationError.validationError({data:{message:validateFacility.error.details[0].context.label}},res);
    }
}
const addFacilityComponent=(req,res,next)=>{
    const validateComponent=accomodation.newFacilityComponent.validate(req.body);
    if (!validateComponent.error) {
        next();
    } else {
        ApplicationError.validationError({data:{message:validateComponent.error.details[0].context.label}},res);
    }
}
...
const updateAccomodation=(req,res,next)=>{
    const validateAccomodation=accomodation.updateAccomodation.validate(req.body);
    if (!validateAccomodation.error) {
        next();
    } else {
        ApplicationError.validationError({data:{message:validateAccomodation.error.details[0].context.label}},res);
...
export {addAccomodation, addfacility,addFacilityComponent, 
    updateFacilityComponent,updateAccomodation,updateFacility}