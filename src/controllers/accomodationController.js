...
const newEntry=async (req,res)=>{
    ...
    try {
        ...
        if(save){
            return res.status(201).json({data:{message: 'Accomodation is saved'}});
        }
...
const getAll=async (req,res)=>{
    try{
        ...
        return res.status(200).json({status: 200, data:{accommodations: accommodationsList}});
    }catch (error){
        ApplicationError.internalServerError({data:{message:`Error: ${error} ,try again!`}},res);
    }
}
const getById=async (req,res)=>{
    try {
        ...
        return res.status(200).json({status: 200, data:{accommodations: result}});
    } catch (error) {
        ApplicationError.internalServerError({data:{message:`Error: ${error} ,try again!`}},res);
    }
...
const doUpdate=async (req,res)=>{
    ...
    try {
        ...
        if (!accommodation) {
            return notFound.isNotFound({data:{message:'Accommodation not found, try again!'}},res);
        }
        if(req.body.name){
        ...
        return res.status(200).json({status: 200, data:{message: 'Record Updated'}});
    } catch (error) {
        ApplicationError.internalServerError({data:{message:`Error, try again!`}},res);
    }
}
