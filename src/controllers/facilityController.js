...
const newfacilityEntry=async (req,res)=>{  
    try {
        ...
        if (accomodationCheck==null) {
            return notFound.isNotFound({data:{message:'Invalid Accomodation, try again!'}},res);
        }
        const checkUser=await userService.findById(req.user.id);
        if (checkUser==null) {
            return notFound.isNotFound({data:{message:'Invalid user, try again!'}},res);
        }
        const addNew=await facilityService.addNew(facility);
        if(addNew){
            return res.status(201).json({status: 201, data:{message: 'Accomodation facility is saved'}});
        }
        ApplicationError.internalServerError({data:{message:'Operation unsuccessful, try again!'}},res);
    } catch (error) {
        ApplicationError.internalServerError({data:{message:`Error,try again!`}},res);
    } 
}
...
const doDelete=async(req,res)=>{
    try {
        ...
        if (!facility) {
            return notFound.isNotFound({data:{message:'Accommodation facility not found, try again!'}},res);
        }
        await facilityService.removeById(req.params.Id);
        return res.status(200).json({status: 200, data:{message: 'Record is Removed'}});
    } catch (error) {
        ApplicationError.internalServerError({data:{message:`Error, try again!`}},res);
    }
}
...
const fetchAllInAccomodation=async (req,res)=>{
    try{
        ...
        return res.status(200).json({status: 200, data:{facilities: facilitiesList}});
    }catch (error){
        ApplicationError.internalServerError({data:{message:`Error, try again!`}},res);
    }
}
...
const doUpdate=async (req,res)=>{
    try {
        ...
        if (!facility) {
            return notFound.isNotFound({data:{message:'Facility not found, try again!'}},res);
        }
        if(req.body.name){
...
const addFacilityComponent=async(req,res)=>{
    ...
    try {
        ...
        if (!facility) {
            return notFound.isNotFound({data:{message:'Invalid facility Id, try again!'}},res);
        }
        const newComponent=await facilityService.addFacilityComponent(component);
        if (newComponent) {
            return res.status(201).json({status: 201, data:{message: 'Facility component is saved'}});
        }
        ApplicationError.internalServerError({data:{message:'Operation unsuccessful, try again!'}},res);
    } catch (error) {
        ApplicationError.internalServerError({data:{message:`Error, try again!`}},res);
...
const getAllcomponents=async(req,res)=>{
    try {
        if (req.params) {
            ...
            if (!facilityId) {
                return notFound.isNotFound({data:{message:`Invalid data, try again!`}},res);
            }
            const allcomponents=await facilityService.getFacilityComponents(facilityId);
            return res.status(200).json({status: 200, data:{allComponents: allcomponents}});   
        }
        return notFound.isNotFound({data:{message:`Invalid data, try again!`}},res);
    } catch (error) {
        ApplicationError.internalServerError({data:{message:`Error, try again!`}},res);
...
const updateComponent=async(req,res)=>{
    try {
        ...
        if (!getComponent) {
            return notFound.isNotFound({data:{message:'Object not found, try again!'}},res);
        }
...
export {
    newfacilityEntry, doDelete,getById, 
    fetchAllInAccomodation, doUpdate,
    addFacilityComponent, getAllcomponents,
